...
const faqs = [
  ...
  },
  {
    ...
    answer: "SafeHive operates seamlessly with your existing robot ecosystem by acting as a high-speed, safety-critical data broker. If your humanoid fleet or AMRs already run Robot Operating System (ROS/ROS2), integration doesn’t require stripping out your existing stack—it simply enhances it.\n\nThe architecture relies on a deterministic, low-latency publish-subscribe model utilizing three primary ROS topics:\n\n1. The Position Topic (Input to Hub): Your agents continuously publish their high-frequency telemetry, spatial coordinates, and localized state data to SafeHive. The Control Hub aggregates these positional streams in real time to map the entire facility's moving canvas.\n\n2. The Telemetry & Reporting Topic (Input to Hub): Beyond raw location, humanoids stream peripheral observations like vision-detected obstacles and system health flags. By cross-referencing what multiple robots and plant sensors see simultaneously, the Hub uses object redundancy to instantly identify and verify known assets—preventing false alarms from \"unidentified objects\" and keeping the line running smoothly.\n\n3. The Command Topic (Output to Agents): Based on the spatial math and verified data processed by the Hub, SafeHive publishes safety-rated speed vectors and dynamic zone commands. These payloads explicitly target only the registered devices currently in or approaching an affected zone.\n\nBy separating the network this way, your machines handle their own local navigation, while SafeHive orchestrates the global, ISO-compliant safety overlay across the entire fleet."
  },
  {
