...
export function SafetyVolumes() {
  ...
  const VolumetricEnvelope = ({ radius, color, isActive, noise }: { radius: number, color: string, isActive: boolean, noise: number }) => {
    const layers = [0, 8, 16, 24]; // Z-offsets for "envelope" feel
    return (
        ...
        <svg viewBox="-150 -150 300 300" className={cn("w-full h-full transition-opacity duration-500", isActive ? "opacity-100" : "opacity-15")}>
          {layers.map((z, idx) => (
            <path
              ...
              fill={idx === 0 ? `${color}15` : 'transparent'}
              stroke={color}
              strokeWidth={isActive ? 2 : 1}
              strokeOpacity={1 - (idx / layers.length)}
              transform={`translate(0, ${-z})`}
              className="transition-all duration-700"
            />
          ))}
          {/* Vertical connectors for 3D feel */}
          <path d="M -140 0 L -140 -24 M 140 0 L 140 -24" stroke={color} strokeWidth="0.5" strokeOpacity="0.2" transform={`scale(${radius / shells.outer})`} />
        </svg>
      </div>
    );
  };
  ...
  const telemetry = [
    { icon: <Ruler className="w-10 h-10" />, value: 'PROXIMITY', subtext: 'Calculates real-time separation distance between the humanoid, the worker, and surrounding machinery.' },
    { icon: <Gauge className="w-10 h-10" />, value: 'SPEED CALIBRATION', subtext: 'Automatically shrinks or expands protective volumes based on the live velocity vectors of different equipment.' },
      ...
      <div className="container mx-auto px-6 relative">
        <div className="max-w-7xl mx-auto space-y-20">
          <div className="space-y-6 text-center max-w-4xl mx-auto">
            <h2 className="text-4xl lg:text-6xl font-headline font-bold tracking-tighter text-slate-900 leading-none">
              DYNAMIC BUBBLE CALCULATION<span className="text-primary">.</span>
            </h2>
            <p className="text-slate-500 text-base lg:text-lg leading-relaxed font-medium px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {telemetry.map((item, idx) => (
              <div key={idx} className="p-10 border border-slate-100 rounded-sm bg-slate-50/50 hover:border-primary/20 hover:bg-white transition-all group flex flex-col h-full shadow-sm hover:shadow-xl hover:shadow-primary/5">
          ...
          <div className="max-w-6xl mx-auto bg-white border border-slate-200 shadow-sm overflow-hidden rounded-sm mt-12">
            <div className="grid lg:grid-cols-10 h-full">
              <div className="lg:col-span-6 bg-white border-b lg:border-b-0 lg:border-r border-slate-100 p-6 lg:p-8 relative min-h-[500px] lg:min-h-[650px] flex flex-col items-center justify-center overflow-hidden">
                <div className="relative w-full flex-1 flex items-center justify-center transition-all duration-700 ease-in-out perspective-[1500px] rotate-x-[60deg] rotate-z-[-45deg] scale-100 lg:scale-110">
                  ...
                  <div className="relative flex items-center justify-center">
                    ...
                    <div className="relative z-40 transition-all duration-700 rotate-z-[45deg] rotate-x-[-90deg] translate-y-[-60px]">
                       <div className="relative h-28 lg:h-40 w-14 flex items-center justify-center">
                ...
                <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-4 bg-white/95 backdrop-blur-md p-6 border border-slate-200 shadow-xl rounded-sm z-[150] mt-4">
                  <div className="space-y-3">
                    ...
                    <Slider value={[proximity]} onValueChange={(v) => setProximity(v[0])} min={50} max={boundaries.rawOuter} step={5} className="py-2" />
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
              ...
              <div className="lg:col-span-4 p-8 lg:p-12 flex flex-col justify-between bg-white">
                <div className="space-y-10">
                  <Tabs value={activeShell} onValueChange={handleTabChange} className="w-full">
                    <TabsList className="w-full h-auto p-0 bg-transparent border-b border-slate-100 rounded-none mb-8 flex justify-between">
